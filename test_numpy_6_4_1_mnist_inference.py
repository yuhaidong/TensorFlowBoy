...
# 在这个程序中将用到dropout方法，dropout可以进一步提升模型可靠性并防止过拟合，dropout过程
# 只在训练时使用。
def inference(input_tensor, train, regularizer):
	# 声明第一层军基层的变量并实现前向传播过程。这个过程和6.3.1小节中介绍的一致。
	...
	with tf.variable_scope('layer3-conv2'):
		...
		# 使用边长为4， 深度为64的过滤器，过滤器移动的步长为1，且使用全0填充。
		conv2 = tf.nn.conv2d(
	...
	# 然后第五层全连接层需要的输入格式为向量，所以在这里需要将这个7*7*64的矩阵拉直成一
	...
	# 输出改为0。dropout可以避免过拟合问题，从而使得训练在测试数据上的效果更好。
	...
	# 输出为一组长度为10的向量。这一层的输出过程Softmax之后就得到了最后的分类结果。
